...
{
    DataCollection* _receiveDataParams;
    DataCollection* _transferDataParams;
    DataCollection* _subTransferDataParams;
    BaseNavigationController* _baseNavigationController;
    id<UIViewPassValueDelegate> _passValueDelegate;
    NSString* _titleText;
    UIImage* _rightBarImage;
    SEL _rightBarButtonOnClick;
    BOOL _isDismissKeyboard;
    //是否隐藏左侧返回按钮，默认不隐藏
    BOOL _isLeftBtnBackHidden;
...
/**
 设置或获取导航栏视图控制器右侧按钮图形
 **/
@property (nullable,strong,nonatomic) IBInspectable UIImage* rightBarImage;
